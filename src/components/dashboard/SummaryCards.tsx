  ...
  if (loading || !data) {
    return (
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="animate-pulse rounded-xl bg-white p-5 shadow-sm border border-gray-200">
      ...
      {cards.map((card) => (
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <span className={`flex h-8 w-8 items-center justify-center rounded-lg ${card.bg} text-sm`}>
              {card.icon}
            </span>
          </div>
